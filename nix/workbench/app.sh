...
app() {
  ...
  case "$op" in
    ...
    compose )
      ...
      local usage="USAGE: wb app $op PROFILE-NAME/JSON NODE-SPECS/JSON NODE_IMAGE_NAME NODE_IMAGE_TAG"
      ...
      yq --argjson nodeImageName "\"$nodeImageName\"" --argjson nodeImageTag "\"$nodeImageTag\"" --yaml-output -f "$global_basedir"/backend/docker-compose.jq $nodespecs;;
