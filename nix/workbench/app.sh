...
app() {
  ...
  case "$op" in
    ...
    compose )
      ...
      yq --argjson nodeImageName "\"$nodeImageName\"" --argjson nodeImageTag "\"$nodeImageTag\"" --argjson tracerImageName "\"$tracerImageName\"" --argjson tracerImageTag "\"$tracerImageTag\"" --yaml-output -f "$global_basedir"/backend/docker-compose.jq $nodespecs;;
